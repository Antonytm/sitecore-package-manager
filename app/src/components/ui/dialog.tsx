 ...
 *
 * `DialogContent` caps its own height, but nothing inside it scrolls — so a body taller
 * than that cap pushed the footer out of the box, where no amount of scrolling could reach
 * it. Putting everything between the header and the footer in here pins both and scrolls
 * only what is between them.
 *
 * `min-h-0` is load-bearing: a flex item defaults to `min-height: auto`, which refuses to
 * shrink below its content and is exactly what defeats the overflow.
 */
